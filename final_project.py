...
def get_poppler_path():
    ...
    for path in possible_paths:
        if os.path.exists(path + "/pdfinfo"):
            return path
...
def select_file():
    ...
    if file_path:
        ...
        drag_label.config(text=f"Selected:\n{os.path.basename(file_path)}")
...
def process_file():
    ...
    # If poppler error happens, show it nicely
    if raw_text.startswith("ERROR"):
        output_box.insert(tk.END, f"\n❌ {raw_text}\n")
        output_box.insert(tk.END, "\nFix: Install poppler with 'brew install poppler'\n")
        return
